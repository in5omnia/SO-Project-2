...
int read_input(char *buffer, char *client_pipe_namepath, char *box_name) {
	...
	memset(client_pipe_namepath + client_pipe_len, '\0',
		   MAX_CLIENTPIPE_NAME - client_pipe_len);    //FIXME check for error
...
int main(int argc, char **argv) {
	...
	client_pipe = start_fifo(client_pipe_name, O_WRONLY);
	while (1) {
		// Read
