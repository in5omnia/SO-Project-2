...
int tfs_open(char const *name, tfs_file_mode_t mode) {
	char *assured_name = assure_pathname(name);
	...
	int inum = tfs_lookup(assured_name, root_dir_inode);
	size_t offset;
	...
	if (inum >= 0) {
		...
		if (add_dir_entry(root_dir_inode, assured_name + 1, inum) == -1) {
			inode_delete(inum);
			if (pthread_mutex_unlock(&g_library_mutex) == -1) {
				WARN("failed to unlock mutex: %s", strerror(errno));
				free(assured_name);
...
int tfs_unlink(char const *target) {
	...
	char * assured_path = assure_pathname(target);

	inode_t *root_dir_inode = inode_get(ROOT_DIR_INUM);
	ALWAYS_ASSERT(root_dir_inode != NULL, "tfs_open: root dir inode must exist");
	int inum = tfs_lookup(assured_path, root_dir_inode);
	...
	inode_delete(inum);
	if (clear_dir_entry(root_dir_inode, assured_path + 1) == -1) {
		if (pthread_mutex_unlock(&g_library_mutex) == -1) {
			WARN("failed to unlock mutex: %s", strerror(errno));
			free(assured_path);
			return -1;
